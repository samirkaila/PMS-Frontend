						...
						<tr v-for=" (user, index) in users" v-bind:index="index" v-bind:key="user.id">
							...
							<td>
								<div class="action-box">
									<router-link class="action-btn edit" :to="{name:'user.detail',params: { id: user.id }}" ><img  :src="`${publicPath}images/icon-edit.svg`" alt=""></router-link>
								</div>
							</td>
						</tr>
					</tbody>
...
export default {
	...
	methods: {
		...
		fatchAvailabilityList(){
			...
			this.isLoading = true
			DataService.userAvailability( data )
			.then(response => {	
