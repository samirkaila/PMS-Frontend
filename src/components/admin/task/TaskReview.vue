			...
			<div class="viewtask-sidebar-block">
				...
				<div class="vtsidebar-databar">
					<div class="vtsidebar-btnbox">
						...
						<a class="vts-btn ml-auto mr-2" href="javascript:void(0)" @click="completeTask(sliderTask.id )">Complate</a>
						<a class="vts-btn " href="javascript:void(0)" @click="unCompleteTask(sliderTask.id )">Uncomplate</a>
					</div>
...
export default {
	...
	methods: {
		...
		fetchUserReviewTasks(){
			...
			this.isLoading = true
			DataService.amdinGetReviwTasks( data )
			.then(response => {	
				if(response.status == 200){
		...
		completeTask(taskID){
			...
			DataService.adminTasksComplete( data )
			.then(response => {	
				if(response.status == 200){
		...
		unCompleteTask(taskID){
			...
			DataService.adminTasksReviewUnComplete( data )
			.then(response => {	
				if(response.status == 200){
					$('#viewtask-sidebar').removeClass("open");
					$("body").removeClass("overflow-hidden");
