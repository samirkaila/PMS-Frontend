	...
	<div class="main_wapper-inner main_wapper-inner-v2">
		...
		<div class="admin-deshboard sidebar">
			...
			<div class="top_add_btn">
				<router-link  :to="{name:'task.create'}" class="h-button task_modal_btnd"> <span><img :src="`${publicPath}images/btn-plus-icon.svg`" alt="" ></span>Add Task </router-link>
			</div>
			...
			<table class="table_main_data" style="width:100%">
				...
				<tbody >
					<tr v-for=" (task, index) in tasks" v-bind:index="index" v-bind:key="task.id">
						<td>{{task.index}}.</td>
						<td data-target-sidebar="viewtask-sidebar"  @click="openTaskSidebar(task._id , task.user.id)">{{task.user.name}}</td>
						<td data-target-sidebar="viewtask-sidebar" @click="openTaskSidebar(task._id , task.user.id)"><div class="text"><strong>{{task.project}} </strong> - {{task.name}}</div></td>
						...
						<td>
							<div class="action-box">
								<!-- <router-link class="action-btn edit" :to="{name:'task.edit',params: { id: task._id }}" ><img  :src="`${publicPath}images/icon-edit.svg`" alt=""></router-link> -->
								<router-link class="action-btn edit" :to="{name:'task.edit',params: { id: task._id }}" ><img  :src="`${publicPath}images/icon-edit.svg`" alt=""></router-link>
			...
			<div class="viewtask-sidebar-block">
				<div class="vtsidebar-topbar">
					<div class="back-arrow">
						<a class="arrow" href="javascript:void(0)" data-dismiss-sidebar="viewtask-sidebar"><img :src="`${publicPath}images/back-arrow.svg`" alt=""></a>
					</div>
				...
				<div class="vtsidebar-databar">
					<div class="vtsidebar-btnbox">
						...
						<!-- <a class="vts-btn ml-auto" href="javascript:void(0)" @click="completeTask(sliderTask.id )">Complate</a> -->
					</div>
...
// import DataTableService from "@/services/DataTables.js";
import $ from 'jquery'
...
export default {
	name: 'AdminHome',components: {
		HeaderAdmin,AdminSideMenu,Loading,
		Paginate
  },data(){
            return{
	...
	methods: {
		...
		fetchUserTasks(){
			...
			this.isLoading = true
			DataService.adminTasksData( data )
			.then(response => {	
				if(response.status == 200){		
