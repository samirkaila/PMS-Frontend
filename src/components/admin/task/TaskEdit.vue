			...
			<div class="create-user">
				<div class="task_form">
					...
					<div class="form_add_tasks">
						<form @submit.prevent="editTask" ref="form" id="task_save_form">
							<div class="row" data-select2-id="select2-data-24-2mss">
								<div class="col-xl-3 col-md-6">
								<div class="input_wrap">
									<label for="">Project</label>
									<multiSelect v-model="this.task.projectID" :options="projectsOpts"  :allow-empty="fasle" @change="projectSelect()" required></multiSelect>
								...
								<div class="col-md-12">
									<div class="input_wrap" id="task_input">
										...
										<div class="input_wrap_file">
											...
											<div class="file_list" v-for="(file, index) in CategoryFiles" v-bind:index="index" v-bind:key="file.id">
												<div class="file-text">{{file.originalname}}</div>
												<div class="download_files">
													<a href="#"></a>
												</div>
												<div class="file_remove"  @click="TaskFileDelete(file,task.id)"></div>
											</div>
										</div>
									</div>
    ...
    Datepicker,Loading,HeaderAdmin,AdminSideMenu
        ...
        methods: {
			...
			fetchEditTask()
			{
				...
				DataService.getTaskAdminDetails( id )
				.then(response => {	
					if(response.status == 200){
						this.isLoading = false
						const data = response.data
						this.CategoryFiles =  data.files
						this.task = {
							id: data._id,
							name: data.name,
			...
			TaskFileDelete(file , id){
				this.isLoading = true
				const data = {
					taskId : id,
					filename:file.filename
				}
				DataService.taskFileDelete( data )
				.then(response => {	
					if(response.status == 200){
						this.isLoading = false
					}
			...
			editTask()
			{
				...
				DataService.taskUpdate( id,data )
				.then(response => {	
					if(response.status == 200){
						this.task = {};
						this.isLoading = false
