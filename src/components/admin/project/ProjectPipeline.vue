				...
				<div class="task-listing  mt-0">
					...
					<tbody>
						<tr v-for=" (project, index) in projects" v-bind:index="index" v-bind:key="project.id" >
							...
							<td data-target-sidebar="viewtask-sidebar">{{project.estimatetime}}</td>
							<td data-target-sidebar="viewtask-sidebar" >{{project.startDate}}</td>
							<td data-target-sidebar="viewtask-sidebar" >{{project.endDate}}</td>
							<td>
	...
	name: 'AdminProject',components: {
        ...
        methods: {
			...
			fetchAllProjects(){
				...
				this.isLoading = true
				DataService.fetchProjectPipeline( data )
				.then(response => {	
					if(response.status == 200){
			...
			pageScript(){
				$(document).ready( () => {
					$("#admin_project_list_table").DataTable();
					$('#project_selecte').select2({
